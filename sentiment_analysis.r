stop=c("Accept","add","admire","admit","advise","afford","agree","alert","bring","extremely","awful","sauce","not","br","put","soon","make","br","perhaps","go","recommend"
       ...
       ,"enter"
       ,"entertain",
       "select","get","inc","jan","name","yes","brule","none"
       ,"escape"
       ,"examine","please"
       ,"excite"
       ,"excuse"
       ...
       "buy"	,"bought"	,"bought",
       "catch",	"caught"	,
       "choose",	"chose",	"chosen","thu","tue",
       "come"	,"came"	,"come",
       "cost",	"service","detailsbh",
       "cut","dug",
       "dive",	"dove",	"dived","brule","xbr","yes",
       "do"	,"did"	,"done",
       "draw",	"drew",	"drawn",
...
clean_data<-clean_text(df,"Comments.and.Work.notes")
...
top_terms_by_topic_LDA <- function(data,corpus, # return a plot? TRUE by defult
                                   k = 4,nstart,best,burnin,seed) # number of topics (4 by default)
{    
  #corpus<-clean_text(data,"Text")
  clean_data <- DocumentTermMatrix(corpus)
  rowTotals <- apply(clean_data , 1, sum) #Find the sum of words in each Document
  dtm.new   <- clean_data[rowTotals> 0, ]
...
topp<-top_terms_by_topic_LDA(df,clean_data,k = 5,nstart,best,burnin,seed)
...
senti=sentiment(data=reviews,Text="Text",topic=1,ldaOut.terms=topp[2],corpus = corpus)


df<-read.csv("/users/navyarao/Downloads/SNOW Data Jan 2018 to July 10 2019.csv")
